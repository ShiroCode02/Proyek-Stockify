    ...
    <div class="py-8 bg-gradient-to-b from-gray-50 dark:from-gray-800 to-white dark:to-gray-900">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-visible shadow-xl sm:rounded-lg p-6">
                <form action="{{ route('stock.storeIn') }}" method="POST" class="space-y-6">
                    @csrf
                    ...
                    <div class="grid gap-6">
                        <div>
                            ...
                            <select name="product_id" class="mt-1 block w-full border border-gray-300 dark:border-gray-600 rounded-lg shadow-sm focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-700" required>
                                ...
                                @foreach($products as $product)
                                    <option value="{{ $product->id }}" {{ $productId && $product->id == $productId ? 'selected' : '' }}>{{ $product->name }} @if($product->supplier) ({{ $product->supplier->name }}) @endif</option>
                                @endforeach
                            </select>
                            @error('product_id')
                        ...
                        {{-- Hapus select supplier, tampilkan di option produk aja kalau ada --}}
                        <div>
                        ...
                        {{-- Tambah optional status select untuk fleksibilitas --}}
                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Status</label>
                            <select name="status" class="mt-1 block w-full border border-gray-300 dark:border-gray-600 rounded-lg shadow-sm focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-700">
                                <option value="pending">Pending (Menunggu Konfirmasi)</option>
                                <option value="completed" selected>Completed (Langsung Update Stok)</option>
                            </select>
                            <p class="text-xs text-gray-500 mt-1">Default berdasarkan role Anda.</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Catatan</label>
                            <textarea name="notes" class="mt-1 block w-full border border-gray-300 dark:border-gray-600 rounded-lg shadow-sm focus:ring-2 focus:ring-blue-400 dark:focus:ring-blue-400 h-24 resize-y text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-700">{{ old('notes') }}</textarea>
                            @error('notes')
                                <p class="text-red-500 dark:text-red-400 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        </div>
